  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-black text-white tracking-tighter uppercase">Fornecedores</h2>
        <button 
          onClick={handleNew}
        ...
        <div className="bg-surface p-6 rounded-2xl shadow-sm border-b-4 border-[#1D9E75] mb-6">
          <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="lg:col-span-2">
              ...
              <input type="email" className="w-full border border-border-hover rounded-md p-2" 
            ...
            <div>
              ...
              <input type="text" maxLength={2} className="w-full border border-border-hover rounded-md p-2 uppercase" 
            ...
            <div className="lg:col-span-4 bg-surface-alt p-3 rounded-lg border border-border">
            ...
            <div className="lg:col-span-4 flex justify-end mt-4 gap-2">
              <button type="button" onClick={() => setIsFormOpen(false)} className="bg-surface-hover text-primary px-6 py-2 rounded-md font-bold uppercase tracking-wider text-sm hover:bg-slate-300">
                Cancelar
              </button>
      ...
      <div className="bg-surface rounded-2xl shadow-md border border-border overflow-hidden flex flex-col">
        <div className="p-4 bg-surface-alt border-b border-border flex justify-between items-center">
            <h4 className="font-black text-white uppercase tracking-wider">Parceiros e Fornecedores</h4>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="text-sm border-t border-border">
              {(data.fornecedores || []).map((f: any) => (
                <tr key={f.id} className="border-b border-border hover:bg-surface-alt cursor-pointer" onClick={() => setSelectedOverviewFornecedor(f)}>
                  <td className="px-4 py-3">
                    <span className="text-primary font-bold uppercase">{f.nome}</span>
                    {f.observacoes && <div className="text-[10px] text-placeholder mt-1 uppercase truncate max-w-[200px]">{f.observacoes}</div>}
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-mono text-muted tracking-wider text-xs">{f.documento || '-'}</div>
                    <div className="font-medium text-muted text-xs mt-1">{f.telefone ? maskPhone(f.telefone) : '-'} {f.email ? `• ${f.email}` : ''}</div>
                  </td>
                  <td className="px-4 py-3">
                     <div className="text-muted text-xs uppercase mb-1">{f.cidade ? `${f.cidade}-${f.estado}` : '-'}</div>
                     {f.isFornecedorViagem && (
                        <span className="bg-blue-900/50 text-blue-300 text-[10px] px-2 py-1 rounded font-black uppercase tracking-wider inline-block">Viagem</span>
                     )}
                  </td>
              ...
              {(!data.fornecedores || data.fornecedores.length === 0) && (
                <tr><td colSpan={4} className="p-4 text-center text-muted font-medium">Nenhum fornecedor cadastrado.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-surface rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-border flex flex-col">
            <div className="p-6 border-b border-border flex justify-between items-center bg-surface-alt sticky top-0 z-10 rounded-t-2xl">
              <div>
                 ...
                 <p className="text-xs font-mono text-muted mt-1">{selectedOverviewFornecedor.nome} | Doc: {selectedOverviewFornecedor.documento || '-'}</p>
              </div>
              <button onClick={() => setSelectedOverviewFornecedor(null)} className="p-2 hover:bg-surface-hover rounded-full transition-colors">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="bg-surface p-5 rounded-xl border border-border shadow-sm flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                      <span className="text-sm font-bold text-primary uppercase">{selectedOverviewFornecedor.nome}</span>
               ...
               <div className="bg-surface p-5 rounded-xl border border-border shadow-sm flex flex-col gap-3">
                  <h4 className="text-[10px] font-black uppercase text-placeholder tracking-widest flex items-center gap-1"><ShoppingCart size={14}/> Histórico de Pagamentos/Gastos</h4>
                      ...
                      {data.contasPagar && data.contasPagar.filter((cp: any) => cp.fornecedor === selectedOverviewFornecedor.nome && cp.status !== 'Cancelado').length > 0 ? (
                        <div className="space-y-3 mt-2 overflow-y-auto max-h-48 mb-2">
                          {data.contasPagar.filter((cp: any) => cp.fornecedor === selectedOverviewFornecedor.nome && cp.status !== 'Cancelado').map((cp: any) => (
                             <div key={cp.id} className="text-xs bg-surface-alt p-2 border border-border rounded flex justify-between">
                                 <div>
                                   <strong className="text-white block mb-1">{cp.categoria} - {cp.status}</strong>
                                   Venc: {new Date(cp.vencimento + 'T12:00:00Z').toLocaleDateString('pt-BR')}
                                 </div>
                                 <strong className="text-primary">{formatCurrency(cp.valor)}</strong>
                             </div>
                          ))}
                        </div>
                      ) : (
                        <p className="text-sm text-muted italic mt-2">Nenhum gasto registrado.</p>
                      )}
